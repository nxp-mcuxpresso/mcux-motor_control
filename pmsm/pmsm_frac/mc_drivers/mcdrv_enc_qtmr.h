...
#ifndef _MCDRV_ENC_QTMR_H_
#define _MCDRV_ENC_QTMR_H_
...
typedef struct _mcdrv_qtmr_enc_t
{
    AMCLIB_TRACK_OBSRV_T_F32 sTo; /* tracking observer structure */
    TMR_Type *pui32QdBase;        /* pointer to QTMR module base address*/

    frac16_t *pf16SpdMeEst;       /* pointer to measured mechanical speed  */
    frac16_t *pf16PosElEst;       /* pointer to measured electrical position */
    frac16_t f16PosErr;           /* position error to tracking observer  */
    acc32_t a32PosMeGain;         /* encoder pulses to mechanical position scale gain */
    frac16_t f16SpdMeEst;         /* estimated speed calculated using tracking observer */
    frac16_t f16SpdElEst;		  /* estimated electrical speed calculated using tracking observer */
    ...
    int32_t i32RevCounter;        /* revolution counter */
    frac32_t ui16PulseNumber;     /* quadrature pulses per one revolution */
...
void MCDRV_QtmrEncSetPosMe(mcdrv_qtmr_enc_t *this, frac16_t f16PosMe);
