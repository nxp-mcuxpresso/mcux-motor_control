...
void MCDRV_FtmPwm3PhSet(mcdrv_pwm3ph_ftm_t *this)
{
    ...
    /* phase A */
    f16DutyCycle                                            = MLIB_Mul_F16((frac16_t)this->pui32PwmBase->MOD, sUABCtemp.f16A);
    this->pui32PwmBase->CONTROLS[this->ui16ChanPhA].CnV     = FTM_CnV_VAL(MLIB_Neg_F16(f16DutyCycle));
    this->pui32PwmBase->CONTROLS[this->ui16ChanPhA + 1U].CnV = FTM_CnV_VAL(f16DutyCycle);

    /* phase B */
    f16DutyCycle                                            = MLIB_Mul_F16((frac16_t)this->pui32PwmBase->MOD, sUABCtemp.f16B);
    this->pui32PwmBase->CONTROLS[this->ui16ChanPhB].CnV     = FTM_CnV_VAL(MLIB_Neg_F16(f16DutyCycle));
    this->pui32PwmBase->CONTROLS[this->ui16ChanPhB + 1U].CnV = FTM_CnV_VAL(f16DutyCycle);

    /* phase C */
    f16DutyCycle                                            = MLIB_Mul_F16((frac16_t)this->pui32PwmBase->MOD, sUABCtemp.f16C);
    this->pui32PwmBase->CONTROLS[this->ui16ChanPhC].CnV     = FTM_CnV_VAL(MLIB_Neg_F16(f16DutyCycle));
    this->pui32PwmBase->CONTROLS[this->ui16ChanPhC + 1U].CnV = FTM_CnV_VAL(f16DutyCycle);
...
bool_t MCDRV_FtmPwm3PhFltGet(mcdrv_pwm3ph_ftm_t *this)
{
    ...
    if((this->pui32PwmBase->FMS & ((1UL << this->ui16FaultFixNum) | (1UL << this->ui16FaultAdjNum))) != 0UL)
    {
        s_statusPass = TRUE;
    }
    else
