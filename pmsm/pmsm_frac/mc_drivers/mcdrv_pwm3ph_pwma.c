...
bool_t MCDRV_eFlexPwm3PhOutEn(mcdrv_pwm3ph_pwma_t *this)
{
    ...
    ui32MaskTemp = (1U << (this->ui16PhASubNum)) | (1U << (this->ui16PhBSubNum)) | (1U << (this->ui16PhCSubNum));
    ...
    this->pui32PwmBaseAddress->OUTEN = (this->pui32PwmBaseAddress->OUTEN & ~(uint16_t)PWM_OUTEN_PWMA_EN_MASK) |
                                       PWM_OUTEN_PWMA_EN(ui32MaskTemp) | (this->pui32PwmBaseAddress->OUTEN);

    /* PWM_B output */
    this->pui32PwmBaseAddress->OUTEN = (this->pui32PwmBaseAddress->OUTEN & ~(uint16_t)PWM_OUTEN_PWMB_EN_MASK) |
                                       PWM_OUTEN_PWMB_EN(ui32MaskTemp) | (this->pui32PwmBaseAddress->OUTEN);
    
    /* Unmask PWM outputs */
#if (defined(FSL_FEATURE_PWM_MASK_HAS_UPDATE_MASK_BITFIELD)) && (FSL_FEATURE_PWM_MASK_HAS_UPDATE_MASK_BITFIELD == 1)
    this->pui32PwmBaseAddress->MASK &= ~(PWM_MASK_MASKA(ui32MaskTemp) | PWM_MASK_MASKB(ui32MaskTemp));
    this->pui32PwmBaseAddress->MASK |= PWM_MASK_UPDATE_MASK(ui32MaskTemp);
#endif
...
bool_t MCDRV_eFlexPwm3PhOutDis(mcdrv_pwm3ph_pwma_t *this)
{
    ...
    /* PWM_A output */
    this->pui32PwmBaseAddress->OUTEN = this->pui32PwmBaseAddress->OUTEN & ~(PWM_OUTEN_PWMA_EN(ui32MaskTemp));

    /* PWM_B output */
    this->pui32PwmBaseAddress->OUTEN = this->pui32PwmBaseAddress->OUTEN & ~(PWM_OUTEN_PWMB_EN(ui32MaskTemp));
...
bool_t MCDRV_eFlexPwm3PhFltGet(mcdrv_pwm3ph_pwma_t *this)
{
    ...
    s_statusPass = (((this->pui32PwmBaseAddress->FAULT[0].FSTS & PWM_FSTS_FFPIN_MASK) >> 8) &
                    (1 << this->ui16FaultFixNum | 1 << this->ui16FaultAdjNum));

    /* clear faults flag */
    this->pui32PwmBaseAddress->FAULT[0].FSTS = ((this->pui32PwmBaseAddress->FAULT[0].FSTS & ~(uint16_t)(PWM_FSTS_FFLAG_MASK)) |
                                       (1 << this->ui16FaultFixNum | 1 << this->ui16FaultAdjNum));
