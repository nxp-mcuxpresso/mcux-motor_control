 ...
 * @return none
 */
void MCDRV_CurrGet(mcdrv_adc_t *this)
{
    GMCLIB_3COOR_T_F16 sIABCtemp;
    
    /* Command 1 */
    this->ui16AdcCurrA = (uint16_t)(((ADC1->RESFIFO) & ADC_RESFIFO_D_MASK) << 1U); 
    /* Command 2 */
    this->ui16AdcCurrB = (uint16_t)(((ADC1->RESFIFO) & ADC_RESFIFO_D_MASK) << 1U);
    /* Command 3 */
    this->ui16AdcCurrC = (uint16_t)(((ADC1->RESFIFO) & ADC_RESFIFO_D_MASK) << 1U);        
...
{
    this->ui16AdcDCBVolt = (ADC0->RESFIFO);
